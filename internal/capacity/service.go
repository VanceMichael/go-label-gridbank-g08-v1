...
func (s *Service) Append(ctx context.Context, principal auth.Principal, streamID, requestID string, inputs []SegmentInput) (AppendResult, error) {
	...
	err := s.db.Write(ctx, func(tx *sql.Tx) error {
		...
		for _, input := range inputs {
			existing, found, err := s.repo.FindSegmentByKey(ctx, tx, principal.TenantID, streamID, input.IdempotencyKey)
			if err != nil {
				return err
			}
			if found {
		...
		if result.Inserted > 0 {
			if err := s.repo.RefreshAggregate(ctx, tx, principal.TenantID, streamID, manifest.Version, storage.FormatTime(now)); err != nil {
				return err
			}
		}
		auditID, err := domain.NewID("audit")
		...
		return s.audits.Append(ctx, tx, audit.Record{ID: auditID, TenantID: principal.TenantID, ActorID: principal.UserID, Action: "capacity.append", ObjectType: "capacity_stream", ObjectID: streamID, Outcome: "accepted", RequestID: requestID, Detail: string(detail), CreatedAt: now})
	})
	if err != nil {
