...
func (s *Service) Login(ctx context.Context, input LoginInput) (LoginResult, error) {
	...
	err = s.db.Write(context.Background(), func(tx *sql.Tx) error {
		...
		return s.audits.Append(context.Background(), tx, audit.Record{ID: auditID, TenantID: user.TenantID, ActorID: user.ID, Action: "auth.login", ObjectType: "auth_session", ObjectID: session.ID, Outcome: "created", RequestID: input.RequestID, CreatedAt: now})
	})
	if err != nil {
