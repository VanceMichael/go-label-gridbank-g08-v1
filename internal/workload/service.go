	...
	err := s.db.Write(ctx, func(tx *sql.Tx) error {
		...
		if found {
			...
			if _, err := tx.ExecContext(ctx, `
				UPDATE leases SET released_at = NULL, updated_at = ?, version = version + 1
				WHERE tenant_id = ? AND id = ?`, storage.FormatTime(now), principal.TenantID, result.Lease.ID); err != nil {
				return fmt.Errorf("restore replay lease: %w", err)
			}
			result.Replay = true
			return nil
		}
