...
func (s *Service) Plan(ctx context.Context, principal auth.Principal, input PlanInput) (PlanResult, error) {
	...
	err := s.db.Write(ctx, func(tx *sql.Tx) error {
		...
		if found {
			...
			if err := s.outbox.Enqueue(ctx, tx, domain.OutboxEvent{
				ID: eventID, TenantID: principal.TenantID, Topic: "workload.plan",
				AggregateType: "workload_session", AggregateID: result.Workload.ID,
				Payload: string(record.Response), Status: domain.OutboxPending,
				MaxAttempts: 5, NextAttemptAt: now, CreatedAt: now, UpdatedAt: now, Version: 1,
			}); err != nil {
				return err
			}
			result.Replay = true
