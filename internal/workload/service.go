...
func (s *Service) Submit(ctx context.Context, principal auth.Principal, workloadID, requestID string) (domain.WorkloadSession, error) {
	...
	err := s.db.Write(transitionCtx, func(tx *sql.Tx) error {
		current, err := s.repo.Find(transitionCtx, tx, principal.TenantID, workloadID)
		if err != nil {
			return err
		}
		...
		manifestCount, err := s.repo.CountStreams(transitionCtx, tx, principal.TenantID, workloadID)
		if err != nil {
			return err
		}
		unaligned, err := s.repo.CountUnalignedStreams(transitionCtx, tx, principal.TenantID, workloadID)
		if err != nil {
			return err
		}
		...
		if err := s.repo.Transition(transitionCtx, tx, principal.TenantID, workloadID, current.Status, domain.WorkloadMetering, current.Version, now, TransitionTimes{SubmittedAt: &now}); err != nil {
			return err
		}
		if err := s.appendEffects(transitionCtx, tx, principal, requestID, "workload.submit", workloadID, "metering", now); err != nil {
			return err
		}
