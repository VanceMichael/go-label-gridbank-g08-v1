...
func (s *Service) Plan(ctx context.Context, principal auth.Principal, input PlanInput) (PlanResult, error) {
	...
	var selectedPool domain.ComputePool
	var selectedOffer domain.CapacityOffer
	...
	if !result.Replay && !selectedPool.Supports(selectedOffer.RequiredCapabilities) {
		return PlanResult{}, domain.Precondition("workload.plan", "compute_pool", selectedPool.ID, "pool does not satisfy capacity_offer capabilities")
	}
	return result, nil
}
