...
func (s *Service) RenewPool(ctx context.Context, principal auth.Principal, leaseID, poolID, token string, version int64, requestID string) (domain.PoolLease, error) {
	...
	lease, err := s.repo.FindActiveLease(detachedCtx, s.db.SQL(), principal.TenantID, poolID)
	if err != nil {
		return domain.PoolLease{}, err
	}
	...
	err = s.db.Write(detachedCtx, func(writeCtxTx *sql.Tx) error {
		if err := s.repo.RenewLease(detachedCtx, writeCtxTx, principal.TenantID, lease.ID, principal.UserID, token, version, now, expires); err != nil {
			return err
		}
		return s.audits.Append(detachedCtx, writeCtxTx, audit.Record{ID: auditID, TenantID: principal.TenantID, ActorID: principal.UserID, Action: "pool.renew", ObjectType: "pool_lease", ObjectID: lease.ID, Outcome: "renewed", RequestID: requestID, CreatedAt: now})
	})
	if err != nil {
		return domain.PoolLease{}, err
	}
