...
func (Repository) ReleaseActiveLeaseForRetry(ctx context.Context, q storage.Queryer, tenantID, poolID string, now time.Time) error {
	_, err := q.ExecContext(ctx, `
		UPDATE leases
		SET released_at = ?, updated_at = ?, version = version + 1
		WHERE tenant_id = ? AND pool_id = ? AND released_at IS NULL`,
		storage.FormatTime(now), storage.FormatTime(now), tenantID, poolID)
	if err != nil {
		return fmt.Errorf("release active pool lease for retry: %w", err)
	}
	return nil
}
