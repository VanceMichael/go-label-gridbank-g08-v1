...
func (s *Service) Reserve(ctx context.Context, principal auth.Principal, accountID, workloadID string, amount int64, key, requestID string) (domain.CreditAccount, error) {
	...
	err = s.db.Write(ctx, func(tx *sql.Tx) error {
		...
		result, err := tx.ExecContext(ctx, `UPDATE credit_accounts SET held = held + ?, updated_at = ?, version = version + 1 WHERE tenant_id = ? AND id = ? AND version = ? AND balance - held >= ?`, amount, storage.FormatTime(now), principal.TenantID, accountID, account.Version, amount)
		if err != nil {
		...
		if err != nil {
			return fmt.Errorf("read reserve credit result: %w", err)
		}
		if changed != 1 {
			return domain.Conflict("credit.reserve", "credit_account", accountID, "another reservation consumed the available balance")
		}
		account.Held += amount
		account.UpdatedAt, account.Version = now, account.Version+1
