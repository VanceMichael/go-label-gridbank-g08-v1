...
func (s *Service) RecoverExpired(ctx context.Context, tenantID, actorID, requestID string) (Result, error) {
	...
	err := s.db.Write(ctx, func(tx *sql.Tx) error {
		...
		if err := requeueExpiredWorkloads(ctx, tx, tenantID, now); err != nil {
			return err
		}
...
func requeueExpiredWorkloads(ctx context.Context, tx *sql.Tx, tenantID string, now time.Time) error {
	_, err := tx.ExecContext(ctx, `
		UPDATE workloads
		SET status = 'failed', updated_at = ?, version = version + 1
		WHERE tenant_id = ? AND status = 'ready'
		  AND id IN (SELECT workload_id FROM leases WHERE tenant_id = ? AND released_at = ?)`,
		storage.FormatTime(now), tenantID, tenantID, storage.FormatTime(now))
	if err != nil {
		return fmt.Errorf("requeue expired workloads: %w", err)
	}
	return nil
}
