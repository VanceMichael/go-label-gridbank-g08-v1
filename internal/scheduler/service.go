...
func (s *Service) Claim(ctx context.Context, principal auth.Principal) (ClaimResult, error) {
	...
	job, err := s.repo.FindDue(ctx, s.db.SQL(), principal.TenantID, now)
	if err != nil {
		return ClaimResult{}, fmt.Errorf("find scheduler job to claim: %w", err)
	}
	if job.AttemptCount >= job.MaxAttempts {
		return ClaimResult{}, domain.Precondition("scheduler.claim", "scheduler_job", job.ID, "job exhausted its attempt budget")
	}
	err = s.db.Write(ctx, func(tx *sql.Tx) error {
		...
		claimed, err := s.repo.FindJob(ctx, tx, principal.TenantID, job.ID)
		if err != nil {
			return err
		}
		...
		attempt := domain.JobAttempt{ID: attemptID, TenantID: principal.TenantID, JobID: job.ID, Attempt: claimed.AttemptCount, WorkerID: principal.UserID, StartedAt: now, Outcome: "running"}
		...
		result = ClaimResult{Job: claimed, Attempt: attempt}
		return nil
	})
	if err != nil {
