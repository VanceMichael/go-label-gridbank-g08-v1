...
func (s *Service) Enqueue(ctx context.Context, principal auth.Principal, releaseID, requestID string) (domain.SchedulerJob, error) {
	...
	err = s.db.Write(enqueueCtx, func(tx *sql.Tx) error {
		release, err := s.ledgers.FindRelease(enqueueCtx, tx, principal.TenantID, releaseID)
		if err != nil {
			return err
		}
		...
		if err := s.repo.InsertJob(enqueueCtx, tx, job); err != nil {
			return err
		}
		return s.appendEffects(enqueueCtx, tx, principal.TenantID, principal.UserID, requestID, "scheduler.enqueue", job.ID, "queued", now)
	})
	if err != nil {
