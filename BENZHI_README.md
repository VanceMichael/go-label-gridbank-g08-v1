...
这是一个基于 Go 实现的后端服务，用于承载 go-label-gridbank-g08-v1 的业务处理、数据管理与稳定运行。
...
./build_benzhi_docker.sh benzhi-task-350-amd64 linux/amd64
./build_benzhi_docker.sh benzhi-task-350-arm64 linux/arm64
docker run -it benzhi-task-350-amd64:latest
docker run -it --platform linux/arm64 benzhi-task-350-arm64:latest
```

## 题目验证命令

1. 预期退出码 0：`go test ./integration -run '^TestConcurrentSchedulerClaimCreatesOneAttempt$' -count=1`
2. 预期退出码 0：`go test ./...`
